defmodule Exosphere.ATProto.MST.Node do
  ...
  A `child` is `nil`, a `%CID{}` (still in the store, not yet loaded), or a
  ...
  """
  @spec load_root(CID.t(), Store.t()) :: {:ok, t()} | {:error, term()}
  def load_root(%CID{} = cid, store) do
    with {:ok, layer} <- derive_layer(cid, store, 0, %{}) do
      load(cid, layer, store)
    end
  end
  ...
  """
  @spec load(CID.t(), integer(), Store.t()) :: {:ok, t()} | {:error, term()}
  def load(%CID{} = cid, layer, store) do
    with {:ok, block} <- Store.fetch(store, cid),
  ...
  """
  @spec resolve(child(), integer(), Store.t()) :: {:ok, t() | nil} | {:error, term()}
  def resolve(nil, _layer, _store), do: {:ok, nil}
  def resolve(%__MODULE__{} = node, _layer, _store), do: {:ok, node}
  def resolve(%CID{} = cid, layer, store), do: load(cid, layer, store)
  ...
  """
  @spec put(t(), binary(), CID.t(), integer(), Store.t()) :: {:ok, t()} | {:error, term()}
  def put(%__MODULE__{} = root, key, %CID{} = value, key_layer, store) do
    root
    |> grow_to(key_layer)
    |> insert(key, value, key_layer, store)
  end
  ...
  """
  @spec delete(t(), binary(), integer(), Store.t()) :: {:ok, t()} | {:error, term()}
  def delete(%__MODULE__{} = root, key, key_layer, store) do
    with {:ok, root} <- remove(root, key, key_layer, store) do
      trim_top(root, store)
    end
  end

  @doc """
  Write every dirty node back to `store`, bottom-up.

  Returns the root's CID, the updated store, and just the blocks this flush
  wrote — which is exactly the set a `#commit` message must carry for the
  nodes it changed.
  """
  @spec flush(t(), Store.t()) ::
          {:ok, CID.t(), Store.t(), %{CID.t() => binary()}} | {:error, term()}
  def flush(%__MODULE__{} = node, store), do: do_flush(node, store, %{})
  ...
  flushed tree.
  """
  @spec path(CID.t(), binary(), Store.t()) :: {:ok, [CID.t()]} | {:error, term()}
  def path(%CID{} = root, key, store) do
    with {:ok, node} <- load_root(root, store) do
      walk_path(node, root, key, store, [])
    end
  end

  defp walk_path(%__MODULE__{layer: layer} = node, cid, key, store, acc) do
    acc = [cid | acc]
    ...
    case Enum.at(node.entries, index) do
      ...
      _ ->
        case span_child(node, index) do
          ...
          %CID{} = child_cid ->
            with {:ok, child} <- load(child_cid, layer - 1, store) do
              walk_path(child, child_cid, key, store, acc)
            end
  ...
  """
  @spec find(CID.t(), binary(), Store.t()) :: {:ok, CID.t() | nil} | {:error, term()}
  def find(%CID{} = root, key, store) do
    with {:ok, node} <- load_root(root, store) do
      walk_find(node, key, store)
    end
  end

  defp walk_find(%__MODULE__{layer: layer} = node, key, store) do
    index = insertion_index(node.entries, key)
    ...
    case Enum.at(node.entries, index) do
      ...
      _ ->
        case span_child(node, index) do
          ...
          child ->
            with {:ok, resolved} <- resolve(child, layer - 1, store) do
              walk_find(resolved, key, store)
            end
        end
    end
  end
  ...
  them.
  """
  @spec neighbor_keys(CID.t(), binary(), Store.t()) ::
          {:ok, {binary() | nil, binary() | nil}} | {:error, term()}
  def neighbor_keys(%CID{} = root, key, store) do
    with {:ok, node} <- load_root(root, store) do
      walk_neighbors(node, key, store, {nil, nil})
    end
  end

  defp walk_neighbors(%__MODULE__{layer: layer} = node, key, store, {prev, next}) do
    index = insertion_index(node.entries, key)
    ...
    case at do
      %{key: ^key} = entry ->
        ...
        with {:ok, deeper_prev} <- rightmost_key(span_child(node, index), layer - 1, store),
             {:ok, deeper_next} <- leftmost_key(entry.tree, layer - 1, store) do
      ...
      _ ->
        case span_child(node, index) do
          ...
          child ->
            with {:ok, resolved} <- resolve(child, layer - 1, store) do
              walk_neighbors(resolved, key, store, {prev, next})
            end
        end
    end
  end
  ...
  """
  @spec leftmost_key(child(), integer(), Store.t()) ::
          {:ok, binary() | nil} | {:error, term()}
  def leftmost_key(nil, _layer, _store), do: {:ok, nil}

  def leftmost_key(child, layer, store) do
    with {:ok, node} <- resolve(child, layer, store) do
      case {node.left, node.entries} do
        {nil, [first | _]} -> {:ok, first.key}
        {nil, []} -> {:ok, nil}
        {left, entries} -> descend_leftmost(left, entries, layer - 1, store)
      end
    end
  end

  defp descend_leftmost(left, entries, layer, store) do
    with {:ok, deeper} <- leftmost_key(left, layer, store) do
  ...
  """
  @spec rightmost_key(child(), integer(), Store.t()) ::
          {:ok, binary() | nil} | {:error, term()}
  def rightmost_key(nil, _layer, _store), do: {:ok, nil}

  def rightmost_key(child, layer, store) do
    with {:ok, node} <- resolve(child, layer, store) do
      case List.last(node.entries) do
        nil ->
          rightmost_key(node.left, layer - 1, store)

        last ->
          with {:ok, deeper} <- rightmost_key(last.tree, layer - 1, store) do
            {:ok, deeper || last.key}
          end
      end
    end
  ...
  defp insert(%__MODULE__{layer: layer} = node, key, value, key_layer, store)
    ...
    case Enum.at(node.entries, index) do
      ...
      _ ->
        splice(node, key, value, index, store)
    end
  end
  ...
  defp insert(%__MODULE__{layer: layer} = node, key, value, key_layer, store)
    ...
    with {:ok, child} <- resolve(span_child(node, index), child_layer, store),
         {:ok, updated} <- insert_into_child(child, key, value, key_layer, child_layer, store) do
      {:ok, put_span_child(node, index, updated)}
    end
  end

  defp insert(%__MODULE__{}, key, _value, _key_layer, _store),
  ...
  defp insert_into_child(nil, key, value, key_layer, from_layer, _store),
    do: {:ok, chain(key, value, key_layer, from_layer)}

  defp insert_into_child(%__MODULE__{} = child, key, value, key_layer, _from, store),
    do: insert(child, key, value, key_layer, store)
  ...
  defp splice(%__MODULE__{layer: layer} = node, key, value, index, store) do
    with {:ok, {left, right}} <- split(span_child(node, index), key, layer - 1, store) do
  ...
  defp remove(%__MODULE__{layer: layer} = node, key, key_layer, store)
       when key_layer == layer do
    case Enum.find_index(node.entries, &(&1.key == key)) do
      ...
      index ->
        ...
        with {:ok, merged} <- merge(span_child(node, index), entry.tree, layer - 1, store) do
          entries =
            node.entries
            |> Enum.take(index)
  ...
  defp remove(%__MODULE__{layer: layer} = node, key, key_layer, store)
    ...
    case resolve(span_child(node, index), child_layer, store) do
      {:ok, nil} ->
        {:error, {:key_not_found, key}}

      {:ok, child} ->
        with {:ok, updated} <- remove(child, key, key_layer, store) do
  ...
  defp remove(%__MODULE__{}, key, _key_layer, _store), do: {:error, {:key_not_found, key}}
  ...
  defp trim_top(nil, _store), do: {:ok, empty()}

  defp trim_top(%__MODULE__{entries: [_ | _]} = node, _store), do: {:ok, node}

  defp trim_top(%__MODULE__{entries: [], left: nil}, _store), do: {:ok, empty()}

  defp trim_top(%__MODULE__{entries: [], left: left, layer: layer}, store) do
    with {:ok, child} <- resolve(left, layer - 1, store) do
      trim_top(child, store)
    end
  end
  ...
  # entry is dropped.
  defp split(nil, _key, _layer, _store), do: {:ok, {nil, nil}}

  defp split(child, key, layer, store) do
    with {:ok, node} <- resolve(child, layer, store) do
      split_node(node, key, layer, store)
    end
  end

  defp split_node(%__MODULE__{} = node, key, layer, store) do
    index = insertion_index(node.entries, key)

    # The subtree at the boundary straddles the key and splits in turn.
    with {:ok, {sub_left, sub_right}} <- split(span_child(node, index), key, layer - 1, store) do
  ...
  defp merge(nil, right, _layer, _store), do: {:ok, right}
  defp merge(left, nil, _layer, _store), do: {:ok, left}

  defp merge(left, right, layer, store) do
    with {:ok, l} <- resolve(left, layer, store),
         {:ok, r} <- resolve(right, layer, store) do
      merge_nodes(l, r, layer, store)
    end
  end

  defp merge_nodes(%__MODULE__{entries: []} = l, %__MODULE__{} = r, layer, store) do
    with {:ok, merged} <- merge(l.left, r.left, layer - 1, store) do
      {:ok, prune(%__MODULE__{layer: layer, left: merged, entries: r.entries})}
    end
  end

  defp merge_nodes(%__MODULE__{} = l, %__MODULE__{} = r, layer, store) do
    with {:ok, merged} <- merge(List.last(l.entries).tree, r.left, layer - 1, store) do
  ...
  defp do_flush(%__MODULE__{} = node, store, written) do
    with {:ok, left, store, written} <- flush_child(node.left, store, written),
         {:ok, entries, store, written} <- flush_entries(node.entries, store, written, []) do
      map = to_map(%{node | left: left, entries: entries})
      bytes = DagCBOR.encode!(map)
      cid = CID.create!(map)

      {:ok, cid, Store.put(store, cid, bytes), Map.put(written, cid, bytes)}
    end
  end

  defp flush_child(nil, store, written), do: {:ok, nil, store, written}
  defp flush_child(%CID{} = cid, store, written), do: {:ok, cid, store, written}

  defp flush_child(%__MODULE__{} = node, store, written),
    do: do_flush(node, store, written)

  defp flush_entries([], store, written, acc), do: {:ok, Enum.reverse(acc), store, written}

  defp flush_entries([entry | rest], store, written, acc) do
    with {:ok, tree, store, written} <- flush_child(entry.tree, store, written) do
      flush_entries(rest, store, written, [%{entry | tree: tree} | acc])
    end
  end
  ...
  # shell root (only reachable from a partial or hostile store) means
  # descending, counting a layer per level; `seen` stops a self-referential
  # store from looping forever.
  defp derive_layer(%CID{} = cid, store, depth, seen) do
    if Map.has_key?(seen, cid) do
      ...
      with {:ok, block} <- Store.fetch(store, cid),
        ...
        case Map.get(map, "e", []) do
          ...
          _ ->
            case link(map, "l") do
              %CID{} = left -> derive_layer(left, store, depth + 1, Map.put(seen, cid, true))
              nil -> {:ok, depth}
            end
        end
      end
