defmodule Exosphere.ATProto.Firehose.Emitter do
  ...
  ## Example
      ...
      # Existing blocks: a map, or `fn cid -> {:ok, bytes} | :error end` for a
      # repository too large to hold in one.
      {:ok, result} =
      ...
      # frames. Persist `result.blocks`, and carry result.root, result.commit
      # and result.rev into the next commit.
  ...
  here:
    ...
    * **Persistence.** Nothing is written anywhere. `result.blocks` holds every
      block this commit produced — MST nodes, records, and the commit — for you
      to store however you store things; `root`, `commit` and `rev` are what the
      next commit builds on.
  ...
  Returns `{:ok, result}` — see `t:result/0`. `:blocks` holds every block the
  commit produced, for you to persist; `:root`, `:commit` and `:rev` are what
  the next commit builds on.
  """
  @spec commit(Blocks.source(), keyword()) :: {:ok, result()} | {:error, term()}
  def commit(source, opts) do
    with {:ok, did} <- require_opt(opts, :did),
         ...
         {:ok, ops, records} <- resolve_writes(writes, previous, source),
         {:ok, root, changed} <- MST.apply_ops(previous, source, tree_ops(ops)),
         # The new root's nodes are in `changed`, not in `source` — the proof
         # walk has to see both.
         {:ok, proof} <-
           MST.covering_proof(
             previous,
             root,
             MST.overlay(source, changed),
             Enum.map(ops, & &1.path)
           ),
      ...
      {:ok,
       %{
         ...
         root: root,
         rev: rev,
         # Everything new this commit produced: MST nodes, record blocks, and
         # the commit block itself. Persist them; nothing was written for you.
         blocks: Map.put(blocks, commit_cid, commit_bytes),
         commit_block: commit
       }}
    end
  end
  ...
  defp resolve_writes(writes, previous, source) do
    Enum.reduce_while(writes, {:ok, [], %{}}, fn write, {:ok, ops, records} ->
      case resolve_write(write, previous, source) do
        {:ok, op, block} ->
  ...
  defp resolve_write({action, path, record}, previous, source)
    ...
    with {:ok, bytes} <- DagCBOR.encode(record),
         ...
         {:ok, prev} <- lookup(previous, source, path) do
  ...
  defp resolve_write({:delete, path}, previous, source) when is_binary(path) do
    with {:ok, prev} <- lookup(previous, source, path) do
      {:ok, %{action: :delete, path: path, cid: nil, prev: prev}, %{}}
    end
  end

  defp resolve_write(write, _previous, _source), do: {:error, {:invalid_write, write}}

  defp lookup(nil, _source, _path), do: {:ok, nil}
  defp lookup(%CID{} = root, source, path), do: MST.fetch(root, source, path)
