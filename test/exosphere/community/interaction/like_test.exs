defmodule Exosphere.Community.Interaction.LikeTest do
  @moduledoc """
  ...
  alias Exosphere.Community.Interaction.Like
  ...
  test "builds from wire-format attrs, decoding strongRef" do
    assert {:ok, %Like{} = like} = Like.new(%{"subject" => @subject, "createdAt" => @created_at})
    assert %StrongRef{} = like.subject
    assert like.subject.uri == @subject["uri"]
    assert like.subject.cid == @subject["cid"]
  ...
  test "to_map/1 includes $type and re-encodes the strongRef subject" do
