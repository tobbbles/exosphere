defmodule Exosphere.ATProto.Firehose.EmitterTest do
  ...
  defp emit(blocks, keypair, opts) do
    Emitter.commit(
      blocks,
      Keyword.merge(
  ...
  describe "commit/2" do
    test "emits a first commit for an empty repository", %{keypair: keypair} do
      assert {:ok, result} =
               emit(%{}, keypair, root: nil, writes: [{:create, path(1), record(1)}])
    ...
    test "a delete carries the record version it removed", %{keypair: keypair} do
      ...
      {:ok, second} =
        emit(first.blocks, keypair, seq: 2, root: first.root, writes: [{:delete, path(1)}])
    ...
    test "the emitted tree agrees with a from-scratch build", %{keypair: keypair} do
      ...
      {:ok, result} = emit(%{}, keypair, root: nil, writes: writes)
    ...
    test "requires the options it cannot invent" do
      assert {:error, {:missing_option, :did}} = Emitter.commit(%{}, seq: 1)

      assert {:error, {:missing_option, :seq}} =
               Emitter.commit(%{}, did: @did)
    end
    ...
    test "refuses more than 200 operations", %{keypair: keypair} do
      ...
      assert {:error, {:too_many_ops, 201}} =
               emit(%{}, keypair, root: nil, writes: writes)
    end
    ...
    test "refuses an oversized record block", %{keypair: keypair} do
      ...
      assert {:error, {:record_too_large, _path, size}} =
               emit(%{}, keypair, root: nil, writes: [{:create, path(1), huge}])
  ...
  describe "a commit chain, verified the way a consumer would" do
    test "every frame inverts to its predecessor's root", %{keypair: keypair} do
      ...
      commits
      |> Enum.with_index(1)
      |> Enum.reduce({nil, %{}}, fn {writes, seq}, {root, blocks} ->
        {:ok, result} = emit(blocks, keypair, seq: seq, root: root, writes: writes)

        assert result.root == verify_as_consumer(result.frame, keypair.public_key, root)

        # Each commit hands back only its new blocks; the repository is what the
        # caller accumulates.
        {result.root, Map.merge(blocks, result.blocks)}
      end)
    end
    ...
    property "randomized commit chains all invert" do
      ...
      check all(
        ...
        batches
        |> Enum.with_index(1)
        |> Enum.reduce({nil, %{}, MapSet.new()}, fn {batch, seq}, {root, blocks, present} ->
          {writes, present} =
            batch
          ...
          {result.root, Map.merge(blocks, result.blocks), present}
        end)
      end
    end
  end
  ...
  describe "sync/1" do
    test "carries only the commit block, rooted at it", %{keypair: keypair} do
      ...
      commit_bytes = result.blocks[result.commit]
