defmodule Exosphere.ATProto.Spaces.Blake3.Reference do
  @moduledoc """
  Pure-Elixir BLAKE3 (unkeyed, XOF mode) — the differential-testing oracle for
  `Exosphere.ATProto.Spaces.Blake3`, which is a NIF.

  This shipped as the production implementation through v0.5.0. It was moved
  into the test tree when the NIF landed: it is ~50x slower than the native
  crate, but it is an independent implementation written directly from the
  BLAKE3 [reference implementation], so agreement between the two is real
  evidence that the NIF is wired up correctly. `blake3_test.exs` checks the
  two against each other across chunk boundaries and output lengths, on top of
  ...
  ## Examples
      ...
      iex> Exosphere.ATProto.Spaces.Blake3.Reference.hash("", 32) |> Base.encode16(case: :lower)
